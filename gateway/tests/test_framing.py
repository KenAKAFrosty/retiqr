...
class TestHdlc:
    ...
    def test_escapes_flag_byte(self):
        ...
        assert 0x7E not in encoded[1:-1]  # flag must not appear raw inside frame
...
class TestHid:
    ...
    def test_only_crc_no_data_rejected(self):
        assert hid_decode(">00<") is None  # too short (< 4 hex chars)
    ...
    def test_non_hex_chars_stripped(self):
        # Keyboard noise (spaces, punctuation) inside the frame must be ignored.
...
class TestQrFragments:
    ...
    def test_reassembly_round_trip(self):
        ...
        chunks = {}
        for f in frags:
            seq, chunk, idx, total = qr_unpack(f)
